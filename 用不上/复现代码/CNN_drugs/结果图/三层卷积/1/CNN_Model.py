...
class Net(nn.Module):
    def __init__(self):
        ...
        self.p1 = nn.MaxPool2d(kernel_size=(2, 2),stride=1) #
        self.c2 = nn.Conv2d(32, 64, kernel_size=(2, 2), stride=1, padding=0) #
        self.c3 = nn.Conv2d(64, 128, kernel_size=(1, 2), stride=1, padding=0) #
        self.p2 = nn.MaxPool2d(kernel_size=(1, 7)) #
        self.l1 = nn.Linear(1152, 256) #全连接层
    ...
    def forward(self,x1,x2,features): # features:1527*1527 行为circRNA, disease, miRNA,列为circRNA, disease, miRNA
        # x1 circ_feature    x2+271  dis_feature  （想一想邻接矩阵）
        x2 = x2 + 271
        ...
        x = self.p1(x)
        # print(f'After p1: {x.shape}')
        x = self.leakyrelu(self.c2(x))
        # print(f'After c2: {x.shape}')
        x = self.p2(x)
        # print(f'After p2: {x.shape}')
        x = self.leakyrelu(self.c3(x))
        # print(f'After c3: {x.shape}')
        x = self.p2(x)
        # print(f'After p2 (second time): {x.shape}')
...
net=Net()
fea,x1,x2=torch.randn(489,489),torch.linspace(0,31,32).long(),torch.linspace(0,31,32).long()
out=net(x1,x2,fea)