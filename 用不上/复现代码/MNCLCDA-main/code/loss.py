...
class Myloss(nn.Module):
    ...
    def forward(self, target, prediction, drug_lap, mic_lap, alpha1, alpha2, sizes, index):
        mask = t.ones_like(target, dtype=bool)
        mask[index] = False
        loss_ls = t.norm((target - prediction), p='fro') ** 2
        # loss_ls = t.norm((target[mask] - prediction[mask]), p='fro') ** 2
