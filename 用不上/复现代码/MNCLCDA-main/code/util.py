...
from pyrwrMaster.pyrwr_code.rwr import RWR
import random
...
def random_index(index_matrix, sizes):
    index_train = []
    index_test = []
    fold_size = index_matrix.shape[1] // sizes.k_fold
    for i in range(sizes.k_fold):
        test_index = index_matrix[:, i * fold_size:(i + 1) * fold_size]
        if i == sizes.k_fold - 1:  # Ensure the last fold includes any remaining samples
            test_index = index_matrix[:, i * fold_size:]
        train_index = np.hstack((index_matrix[:, :i * fold_size], index_matrix[:, (i + 1) * fold_size:]))
...
def crossval_index(drug_mic_matrix, sizes):
    ...
    neg_index_train, neg_index_test = random_index(neg_index_matrix, sizes)
    pos_index_train, pos_index_test = random_index(pos_index_matrix, sizes)
    for i in range(sizes.k_fold):
        # 按列合并neg_index和pos_index
        k_fold_index_train = np.hstack((pos_index_train[i], neg_index_train[i]))
        k_fold_index_test = np.hstack((pos_index_test[i], neg_index_test[i]))
        index.append([k_fold_index_train, k_fold_index_test])
    return index
...
def formart_DISandRNA(RNA_CIS, Disease_DIS):
    ...
    np.savetxt('MNCLCDA-main\data\RWR_FORMART_DIS.csv', DIS)
    np.savetxt('MNCLCDA-main\data\RWR_FORMART_CIS.csv', RNA)
    return DIS, RNA

def SimtoRWR(RNA_CIS, Disease_DIS, FLAGS):
    RNA_CIS = simabs(RNA_CIS)
    Disease_DIS = simabs(Disease_DIS)
    path_DIS = 'MNCLCDA-main\data\RWR_FORMART_DIS.csv'
    path_CIS = 'MNCLCDA-main\data\RWR_FORMART_CIS.csv'
    formart_DISandRNA(RNA_CIS, Disease_DIS)
    RWRRNA = np.array(RWR_Comp(path_CIS, RNA_CIS.shape[0], FLAGS, NAME='RSim')) # 271
    RWRDIS = np.array(RWR_Comp(path_DIS, Disease_DIS.shape[0], FLAGS, NAME='DSim')) # 218
