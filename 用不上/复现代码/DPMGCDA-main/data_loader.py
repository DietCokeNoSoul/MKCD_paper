...
def split_dataset(assMatrix, ccs, dds, k, negr):  #5 cross  5折
    ...
    for i in range(k):
        ...
        #正负样本组成训练集
        trainData = [pos_Sample, neg_Sample] #trainData:2*6608,一列对应一个正例索引的行列号
        #第i折正负样本作为测试集
        testData = [pos_index[:, (i * crossCount):((i + 1) * crossCount)], neg_index[:, ((k + 1 + i) * crossCount * negr):((k + 2 + i) * crossCount * negr)]]#testData:2*51640,一列对应一个负例索引的行列号
        #当前训练集 测试集 分别存储
...
torch.save([cd, feas, trainSet, testSet], 'DPMGCDA_1_1_util.pth')
'''===========一次性计算==========='''
