...
def mask_path(edge_index: Tensor, p: float = 0.3, walks_per_node: int = 1,  walk_length: int = 3, num_nodes: Optional[int] = None, start: str = 'node', is_sorted: bool = False, training: bool = True) -> Tuple[Tensor, Tensor]:
    if p < 0. or p > 1.:
...
class MaskPath(nn.Module):
    def __init__(self, p: float = 0.7, walks_per_node: int = 1, walk_length: int = 3, start: str = 'node', num_nodes: Optional[int]=None, undirected: bool=True):
        super().__init__()
        self.p = p
    ...
    def forward(self, edge_index):
        remaining_edges, masked_edges = mask_path(edge_index, self.p, walks_per_node=self.walks_per_node, walk_length=self.walk_length, start=self.start, num_nodes=self.num_nodes)
        if self.undirected:
            remaining_edges = to_undirected(remaining_edges)
        ''''为什么masked_edges不做无向图处理？'''
