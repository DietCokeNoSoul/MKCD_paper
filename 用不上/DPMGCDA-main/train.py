...
def create_model(maskgae_args,ifGraph=False):
    if maskgae_args.mask == 'Path':
        mask = MaskPath(p=maskgae_args.p, 
                        num_nodes=maskgae_args.c_node_num + maskgae_args.d_node_num,
    ...
    edge_decoder = EdgeDecoder(maskgae_args.hidden_channels, 
                               maskgae_args.decoder_channels,
                                num_layers=maskgae_args.decoder_layers, 
                                dropout=maskgae_args.decoder_dropout)
    if ifGraph:
        feature_extracter = FeatureExtracter(maskgae_args.in_size, 
                                             maskgae_args.feature_size)
        encoder = GNNEncoder(maskgae_args.feature_size[0], 
                             maskgae_args.encoder_channels, 
                             maskgae_args.hidden_channels,
                             num_layers=maskgae_args.encoder_layers, 
                             dropout=maskgae_args.encoder_dropout,
                             bn=maskgae_args.bn, 
                             layer=maskgae_args.layer, 
                             activation=maskgae_args.encoder_activation)
    else:
        feature_extracter=None
        encoder = GNNEncoder(maskgae_args.in_size[0], 
                             maskgae_args.encoder_channels, 
                             maskgae_args.hidden_channels,
                             num_layers=maskgae_args.encoder_layers, 
                             dropout=maskgae_args.encoder_dropout,
...
def association_linkpred(maskgae_args, splits, c_snf_feature,d_snf_feature,homo_graph,c_graph_feature, d_graph_feature, device="cpu"):
    snf_feature_model=create_model(maskgae_args,False)
    graph_feature_model=create_model(maskgae_args,True)

    snf_optimizer = torch.optim.Adam(snf_feature_model.parameters(), lr=maskgae_args.lr, weight_decay=maskgae_args.weight_decay)
    graph_optimizer = torch.optim.Adam(graph_feature_model.parameters(), lr=maskgae_args.lr, weight_decay=maskgae_args.weight_decay)
    ...
    for epoch in tqdm(range(1, 1 + maskgae_args.epochs)):
        loss = graph_feature_model.train_step(train_data,graph_optimizer, homo_graph,node_graph_features, batch_size=maskgae_args.batch_size)
    print('Combined Feature Level Perspective Training!')
    for epoch in tqdm(range(1, 1 + maskgae_args.epochs)):
        loss = snf_feature_model.train_step(train_data, snf_optimizer, homo_graph, node_snf_features, batch_size=maskgae_args.batch_size)
