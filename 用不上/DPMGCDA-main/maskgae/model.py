...
class FeatureExtracter(nn.Module):
    def __init__(self, in_size,feature_size):
        ...
        for i in range(0, len(feature_size)):
            self.homo_layers.append(
                GConv(in_size[i],feature_size[i]))

    def forward(self, homo_graph, origin_feature):
        homo_feature_d = self.homo_layers[0](homo_graph[0],origin_feature[0])
        homo_feature_c = self.homo_layers[1](homo_graph[1], origin_feature[1])
...
class DPMGCDA(nn.Module):
    def __init__(
        ...
        #self.negative_sampler = select_asso_negative_sampler
    ...
    def train_step(self, data, optimizer, homo_graph,node_features,
        batch_size=2 ** 16, grad_norm=1.0
    ):
        self.train()

        if self.feature_extracter==None:
            d_embedding, c_embedding = node_features[0], node_features[1]
        else:
    ...
    @torch.no_grad()
    def test_step(self, data, pos_edge_index, neg_edge_index, homo_graph,node_features,batch_size=2**16):
        self.eval()
        if self.feature_extracter == None:
            d_embedding, c_embedding = node_features[0], node_features[1]
        else:
