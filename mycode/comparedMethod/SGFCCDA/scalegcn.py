...
class ScaleGCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, use_softmax=True):
    ...
    def dump_intermediate_results(self, x, edge_index):
...
class BiScaleGCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, use_softmax=False):
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = F.relu(self.init_layer(x))

        N = x.size()[0]
        temp = x.view(N, 1, self.hidden_channels)
        layers_out = temp

        for i in range(self.num_layers):
            x = self.convs[i](x, edge_index) # error
            x = F.leaky_relu(x, negative_slope=0.1)
            temp = x.view(N, 1, self.hidden_channels)
            layers_out = torch.cat((layers_out, temp), axis=1)

        if use_softmax:
            return F.log_softmax(layers_out, dim=1)
        else:
            return layers_out
