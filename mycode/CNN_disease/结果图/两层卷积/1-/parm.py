...
class Net(nn.Module):
    def __init__(self):
        ...
        #MaxPool2d(池化窗口大小,步长)
        self.c1 = nn.Conv2d(1, 32, kernel_size=(2,2), stride=1, padding=0) #输入通道数1,输出通道数32,卷积窗口大小2*2,步长1,补充0圈
        self.p1 = nn.MaxPool2d(kernel_size=(1, 4)) #池化窗口大小1*4,步长1
        self.c2 = nn.Conv2d(32, 64, kernel_size=(1, 4), stride=1, padding=0) #输入通道数32,输出通道数64,卷积窗口大小1*4,步长1,补充0圈
        self.p2 = nn.MaxPool2d(kernel_size=(1, 7)) #池化窗口大小1*7,步长1
        self.l1 = nn.Sequential(nn.Linear(54*64, 1024), nn.LeakyReLU(), nn.Dropout(0.5), nn.Linear(1024, 256), nn.LeakyReLU()) #全连接层
    ...
    def forward(self,x1,x2,features): # features:1527*1527 行为circRNA, disease, miRNA,列为circRNA, disease, miRNA
        ...
        x = self.p2(self.leakyrelu(self.c2(x)))  #[32, 32, 1, 22]
