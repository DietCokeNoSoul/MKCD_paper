...
'''
    对比10,第二层卷积进行了填充，导致最后输出到全连接层的神经元变多，全连接层规模导致整个模型训练时间变长
    另外在l1和l2之间添加多一层全连接层，对模型效果没有提升
'''