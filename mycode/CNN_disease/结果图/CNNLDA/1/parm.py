...
class Net(nn.Module):
    def __init__(self):
        ...
        #MaxPool2d(池化窗口大小,步长)
        self.c1 = nn.Conv2d(1, 8, kernel_size=(2,2), stride=1, padding=1) #输入通道数1,输出通道数32,卷积窗口大小2*2,步长1,补充0圈
        self.p1 = nn.MaxPool2d(kernel_size=(2, 2),stride=1) #池化窗口大小1*4,步长1
        self.c2 = nn.Conv2d(8, 16, kernel_size=(2, 2), stride=1, padding=1) #输入通道数32,输出通道数64,卷积窗口大小1*4,步长1,补充0圈
        self.l1 = nn.Sequential(nn.Linear(48864, 1024), nn.LeakyReLU(), nn.Dropout(0.5), nn.Linear(1024, 256), nn.LeakyReLU()) #全连接层
