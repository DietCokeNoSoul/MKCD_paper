...
if __name__ == '__main__':
    now = datetime.now() # 显示当前时间
    now = now.strftime("%m_%d-%H_%M_%S") # 格式化时间
    warnings.filterwarnings('ignore') # ignore warnings

    args = gene_arg() # 生成参数
    set_config(args) # 设置参数
    ...
    model = Gradformer(args=args, node_dim=num_features, edge_dim=num_edge_features, num_tasks=num_tasks, mpnn=args.mpnn, pool=args.pool).to(device)
