    ...
    def __init__(self, args, node_dim: int, edge_dim: int, num_tasks: int, mpnn: str, pool: str):
        ...
        if mpnn == 'GIN':
            for _ in range(args.num_layers):
                nn = Sequential(Linear(args.channels, args.channels),
                                ReLU(),
                                Linear(args.channels, args.channels),)
                conv = GPSConv(args.channels, GINConv(nn), heads=args.nhead, dropout=args.dropout, attn_dropout=args.attn_dropout, drop_prob=args.drop_prob)
            ...
            for _ in range(args.num_layers):
                nn = Sequential(Linear(args.channels, args.channels),
                                ReLU(),
                                Linear(args.channels, args.channels),)
                conv = GPSConv(args.channels, GINEConv(nn), heads=args.nhead, dropout=args.dropout, attn_dropout=args.attn_dropout, drop_prob=args.drop_prob)
                self.convs.append(conv)
        elif mpnn == 'GCN':
            Lap_pe = True if "hiv" in args.dataset else False
            for _ in range(args.num_layers):
                conv = GPSConv(args.channels, GatedGCNLayer(args.channels, args.channels, 0, True, equivstable_pe=Lap_pe), heads=args.nhead, dropout=args.dropout, attn_dropout=args.attn_dropout, drop_prob=args.drop_prob)
                self.convs.append(conv)

        # MLP
        self.mlp = Sequential(Linear(args.channels, args.channels // 2),
                            ReLU(),
